...
import { obsoleteElements, obsoleteAttributes } from 'obsohtml';
...
// Pre-built Sets for O(1) obsolete HTML lookups (used by `reportObsoleteHTML`)
const obsoleteElementSet = new Set(obsoleteElements);
const obsoleteAttributeSet = new Set(obsoleteAttributes);
 ...
 *  When true, enables SVG minification using [SVGO](https://www.npmjs.com/package/svgo).
 ...
 *  Default: `false`
 *
 * @prop {boolean | ((type: 'element' | 'attribute', name: string) => void)} [reportObsoleteHTML]
 *  When true, reports obsolete and proprietary HTML elements and attributes via
 *  `options.log`, using [ObsoHTML](https://www.npmjs.com/package/obsohtml).
 *  When a function is provided, it is called with the type (`'element'` or
 *  `'attribute'`) and the name for each occurrence found.
 *
 *  Default: `false`
 *
...
async function minifyHTML(value, options, partialMarkup) {
  ...
  const parser = new HTMLParser(value, {
    ...
    start: async function (tag, attrs, unary, unarySlash, autoGenerated) {
      ...
      if (options.reportObsoleteHTML && !options.insideForeignContent) {
        const report = typeof options.reportObsoleteHTML === 'function'
          ? options.reportObsoleteHTML
          : (type, name) => options.log(`Warning: obsolete HTML ${type} <${name}>`);
        if (obsoleteElementSet.has(lowerTag)) {
          report('element', lowerTag);
        }
        for (const { name } of attrs) {
          if (obsoleteAttributeSet.has(name.toLowerCase())) {
            report('attribute', name.toLowerCase());
          }
        }
      }
