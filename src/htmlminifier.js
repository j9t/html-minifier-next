...
function parseRemoveEmptyElementsUnless(input, options) {
  ...
  return input.map(item => {
    if (typeof item === 'string') {
      ...
      if (!spec && options.log) {
        options.log('Warning: Unable to parse “removeEmptyElementsUnless” specification: "' + item + '"');
      }
      return spec;
    }
    if (options.log) {
      options.log('Warning: “removeEmptyElementsUnless” specification must be a string, received: ' + typeof item);
    }
    return null;
  }).filter(Boolean);
}
  ...
  const parser = new HTMLParser(value, {
    ...
    start: async function (tag, attrs, unary, unarySlash, autoGenerated) {
      ...
      if (options.removeEmptyElements && isElementEmpty && canRemoveElement(tag, attrs)) {
        ...
        if (!shouldPreserveEmptyElement(tag, normalizedAttrs, removeEmptyElementsUnless)) {
