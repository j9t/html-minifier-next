...
function parseRemoveEmptyElementsUnless(input, options) {
  ...
  return input.map(item => {
    if (typeof item === 'string') {
      return parseElementSpec(item, options);
    }
    return null;
  }).filter(Boolean);
}
...
async function minifyHTML(value, options, partialMarkup) {
  ...
  // Parse `removeEmptyElementsUnless` option
  const removeEmptyElementsUnless = parseRemoveEmptyElementsUnless(options.removeEmptyElementsUnless || [], options);
  ...
  const parser = new HTMLParser(value, {
    ...
    start: async function (tag, attrs, unary, unarySlash, autoGenerated) {
      ...
      if (options.removeEmptyElements && isElementEmpty && canRemoveElement(tag, attrs) && !shouldPreserveEmptyElement(tag, attrs, removeEmptyElementsUnless)) {
 ...
 *  Default: `[/<%[\s\S]*?%>/, /<\?[\s\S]*?\?>/]`
 *
 * @prop {string[]} [removeEmptyElementsUnless]
 *  When `removeEmptyElements` is enabled, specifies elements that should be
 *  preserved even when empty. Accepts tag names or HTML-like markup.
 *
 *  Examples:
 *  - Simple tag names: `["td", "span"]`
 *  - HTML markup with attributes: `["<span aria-hidden='true'>", "<td>"]`
 *  - Unquoted attributes: `["<span aria-hidden=true>"]`
 *
 *  When attributes are specified in markup, all specified attributes must
 *  match for the element to be preserved. Additional attributes on the element
 *  are allowed.
 *
 *  Default: `[]`
 *
