...
describe('HTML', () => {
  ...
  test('Parses non-trivial markup', async () => {
    let input, output;

    assert.strictEqual(await minify('</td>'), '');
    assert.strictEqual(await minify('</p>'), '</p>');
    assert.strictEqual(await minify('</p>', { includeAutoGeneratedTags: true }), '<p></p>');
    ...
    assert.strictEqual(await minify('<p id=""class=""title="">x'), '<p id="" class="" title="">x');
  ...
  test('Deduplicates attributes', async () => {
  ...
  test('Space normalization around text', async () => {
    ...
    await Promise.all([
      ...
      const wrappedInput = '<div>' + inputs[0] + '</div>';
      assert.strictEqual(await minify(wrappedInput, { collapseWhitespace: true, conservativeCollapse: true }), wrappedInput, wrappedInput);
      const wrappedOutput = '<div>' + inputs[1] + '</div>';
      assert.strictEqual(await minify(wrappedInput, { collapseWhitespace: true }), wrappedOutput, wrappedInput);
    }));
    ...
    await Promise.all([
      ...
      ' a <? b ?> c<!-- d --> ',
      ' a <? b ?> c <!-- d -->'
    ].map(async function (markup) {
      assert.strictEqual(await minify(markup, { collapseWhitespace: true, conservativeCollapse: true }), markup, markup);
      assert.strictEqual(await minify(markup, { collapseWhitespace: true, removeComments: true }), 'a <? b ?> c', markup);
      assert.strictEqual(await minify(markup, { collapseWhitespace: true, conservativeCollapse: true, removeComments: true }), ' a <? b ?> c ', markup);
      markup = '<p>' + markup + '</p>';
      assert.strictEqual(await minify(markup, { collapseWhitespace: true, conservativeCollapse: true }), markup, markup);
      assert.strictEqual(await minify(markup, { collapseWhitespace: true, removeComments: true }), '<p>a <? b ?> c</p>', markup);
      assert.strictEqual(await minify(markup, { collapseWhitespace: true, conservativeCollapse: true, removeComments: true }), '<p> a <? b ?> c </p>', markup);
    }));
  ...
  test('Handles Conditional Comments syntax (proprietary and deprecated) as regular comments', async () => {
    let input;
  ...
  test('Handles downlevel-revealed Conditional Comments', async () => {
  ...
  test('Removes comments from styles', async () => {
    let input, output;
  ...
  test('Removes CDATA sections from scripts and styles', async () => {
    let input, output;
  ...
  test('Cleans `class` and `style` attributes', async () => {
    let input, output;
  ...
  test('Cleans URI-based attributes', async () => {
    let input, output;
  ...
  test('Cleans number-based attributes', async () => {
    let input, output;
  ...
  test('Cleans other attributes', async () => {
    let input, output;
    ...
    assert.strictEqual(await minify(input), output);
    assert.strictEqual(await minify(input, { includeAutoGeneratedTags: true }), '<body onload="foo();   bar() ;"><p>x</p></body>');
  });

  test('Removes redundant attributes: `<form method="get" …>`', async () => {
    let input;
  ...
  test('Removes redundant attributes: `<input type="text" …>`', async () => {
    let input;
  ...
  test('Removes redundant attributes: `<a name="…" id="…" …>`', async () => {
    let input;
  ...
  test('Removes redundant attributes: `<script src="…" charset="…">`', async () => {
    let input, output;
  ...
  test('Removes redundant attributes: `<… language="javascript" …>`', async () => {
    let input;
  ...
  test('Removes optional tags with attribute value defaults', async () => {
  ...
  test('Removes `media="all"` and minifies CSS', async () => {
  ...
  test('Removes JavaScript-related `type` attributes', async () => {
    let input, output;
  ...
  test('Removes CSS-related `type` attributes', async () => {
    let input, output;
  ...
  test('Warns about unknown options', async () => {
  ...
  test('Removes attribute quotes', async () => {
    let input;
  ...
  test('Keeps attribute quotes needed for special characters', async () => {
    const testCases = [
  ...
  test('Removes attribute quotes for safe values', async () => {
    const testCases = [
  ...
  test('Removes attribute quotes around URL edge cases', async () => {
    const testCases = [
  ...
  test('Removes attribute quotes from values ending in a slash', async () => {
  ...
  test('Preserves custom attribute-wrapping markup', async () => {
  ...
  test('Preserves custom attribute-joining markup', async () => {
    let input;
  ...
  test('Collapses boolean attributes', async () => {
    let input, output;
  ...
  test('Collapses enumerated attributes', async () => {
    ...
    assert.strictEqual(await minify('<img crossorigin="anonymous">', { collapseBooleanAttributes: true }), '<img crossorigin>');
    assert.strictEqual(await minify('<img crossorigin="Anonymous">', { collapseBooleanAttributes: true }), '<img crossorigin>');
    ...
    assert.strictEqual(await minify('<script crossorigin="anonymous" src="x.js"></script>', { collapseBooleanAttributes: true }), '<script crossorigin src="x.js"></script>');
    assert.strictEqual(await minify('<link rel="stylesheet" crossorigin="anonymous" href="x.css">', { collapseBooleanAttributes: true }), '<link rel="stylesheet" crossorigin href="x.css">');
  ...
  test('Reads a slash on a start tag the way HTML does', async () => {
    // HTML ignores the slash, so the element holds what follows until its own end tag
    assert.strictEqual(await minify('<div/>x</div>y'), '<div>x</div>y');
    assert.strictEqual(await minify('<my-comp/>a</my-comp>b'), '<my-comp>a</my-comp>b');
    assert.strictEqual(await minify('<section/><div>a</div></section>b'), '<section><div>a</div></section>b');
    assert.strictEqual(await minify('<a/>x</a>y', { removeEmptyElements: true }), '<a>x</a>y');
    assert.strictEqual(await minify('<div/>a', { includeAutoGeneratedTags: true }), '<div>a</div>');
    assert.strictEqual(await minify('<table><tr><td/>a</table>', { includeAutoGeneratedTags: true }), '<table><tr><td>a</td></tr></table>');

    // An element whose content is text still holds text
    assert.strictEqual(await minify('<title/>a</title>b'), '<title>a</title>b');
    assert.strictEqual(await minify('<textarea/>a</textarea>b'), '<textarea>a</textarea>b');
    assert.strictEqual(await minify('<script/>a</script>b'), '<script>a</script>b');

    // Whitespace is collapsed against the nesting a browser sees
    assert.strictEqual(await minify('<div/> <span>a</span> </div>', { collapseWhitespace: true }), '<div><span>a</span></div>');

    // Void elements are untouched
    assert.strictEqual(await minify('<br/>x'), '<br>x');
    assert.strictEqual(await minify('<img src="a"/>x'), '<img src="a">x');
  });

  test('`keepClosingSlash` reads a slash on a start tag as closing the element', async () => {
    assert.strictEqual(await minify('<div/>x</div>y', { keepClosingSlash: true }), '<div/>xy');
    assert.strictEqual(await minify('<title/>a</title>b', { keepClosingSlash: true }), '<title/>ab');
    // Templates that write their components as self-closing keep both slash and nesting
  ...
  test('Removes optional tags', async () => {
    let input, output;
  ...
  test('Removes optional tags around whitespace', async () => {
    let input, output;
  ...
  test('Removes optional tags with `preserveLineBreaks`', async () => {
    let input, output;
  ...
  test('Removes optional tags in tables', async () => {
    let input, output;
    ...
    // With `includeAutoGeneratedTags: true`, re-minifying is idempotent
    assert.strictEqual(await minify(output, { collapseWhitespace: true, removeOptionalTags: true, includeAutoGeneratedTags: true }), output);

    // Without `includeAutoGeneratedTags`, `<tbody>` is preserved because preceding `</thead>` was omitted
    assert.strictEqual(await minify(output, { collapseWhitespace: true, removeOptionalTags: true }), output);

    // Without `<tbody>` in source, re-minifying is also idempotent
    const noTbodyOutput = '<table>' +
      '<thead><tr><th>foo<th>bar<th>baz' +
      '<tr><td>boo<td>moo<td>loo' +
      '<tfoot><tr><th>baz<th>qux<td>boo' +
      '</table>';
    assert.strictEqual(await minify(noTbodyOutput, { collapseWhitespace: true, removeOptionalTags: true }), noTbodyOutput);

    // Explicit `<tbody>` after `<thead>` without closing tags must be preserved
    input = '<table id=entries>' +
      '<thead><tr><th scope=col>Entry<th scope=col>Date<th scope=col>#' +
      '<tbody><tr><td>data<td><time>Jan 1</time><td>1' +
      '</table>';
    assert.strictEqual(await minify(input, { collapseWhitespace: true, removeOptionalTags: true }), input);
    assert.strictEqual(await minify(input, { removeOptionalTags: true }), input);
  ...
  test('Removes optional tags in description lists', async () => {
    let input, output;
  ...
  test('Removes optional tags in options', async () => {
    let input, output;
  ...
  test('Phrasing content', async () => {
    ...
    assert.strictEqual(await minify(input), output);
    assert.strictEqual(await minify(input, { includeAutoGeneratedTags: true }), '<p>a</p><div>b</div>');
  ...
  test('`ul` and `ol` are treated as phrasing content', async () => {
    let input, output;

    input = '<p>a<ul><li>item</li></ul>';
    output = '<p>a<ul><li>item</li></ul>';
    assert.strictEqual(await minify(input), output);
    assert.strictEqual(await minify(input, { includeAutoGeneratedTags: true }), '<p>a</p><ul><li>item</li></ul>');
  ...
  test('Escapes `script` end tag', async () => {
  ...
  test('Merges consecutive inline scripts', async () => {
    let input, output;
  ...
  test('Collapses inline tag whitespace', async () => {
    let input, output;
  ...
  test('Collapses whitespace between form controls with `collapseInlineTagWhitespace`', async () => {
    let input, output;
  ...
  test('`trimWhitespace`', () => {
    ...
    // A no-break space is content, not whitespace to trim
    assert.strictEqual(trimWhitespace('\xA0a\xA0'), '\xA0a\xA0');
    assert.strictEqual(trimWhitespace(' \xA0a\xA0 '), '\xA0a\xA0');
  ...
  test('`collapseWhitespaceAll`', () => {
    ...
    // A standalone tab survives, a tab inside a run does not
    assert.strictEqual(collapseWhitespaceAll('a\tb'), 'a\tb');
    assert.strictEqual(collapseWhitespaceAll('a \t b'), 'a b');
    assert.strictEqual(collapseWhitespaceAll('a\t\tb'), 'a b');

    // A run holding a no-break space keeps it, with a space per side group
    assert.strictEqual(collapseWhitespaceAll('a \xA0 b'), 'a \xA0 b');
    assert.strictEqual(collapseWhitespaceAll('a  \xA0\xA0  b'), 'a \xA0\xA0 b');
    assert.strictEqual(collapseWhitespaceAll('a\xA0b'), 'a\xA0b');

    // Nothing to collapse
    assert.strictEqual(collapseWhitespaceAll('a b c'), 'a b c');
  ...
  test('`collapseWhitespace`', () => {
    ...
    // Nothing to trim or collapse
    assert.strictEqual(collapseWhitespace('  a  ', plain, false, false, false), '  a  ');

    // Each side trims only when asked
    assert.strictEqual(collapseWhitespace('  a  ', plain, true, false, false), 'a  ');
    assert.strictEqual(collapseWhitespace('  a  ', plain, false, true, false), '  a');
    assert.strictEqual(collapseWhitespace('  a  ', plain, true, true, false), 'a');

    // Trimming leaves a no-break space in place
    assert.strictEqual(collapseWhitespace(' \xA0 a \xA0 ', plain, true, true, false), '\xA0 a \xA0');
    assert.strictEqual(collapseWhitespace('  \xA0\xA0  ', plain, true, true, false), '\xA0\xA0');

    // Conservative collapse leaves one space rather than none
    assert.strictEqual(collapseWhitespace('  a  ', conservative, true, true, false), ' a ');
    assert.strictEqual(collapseWhitespace('\ta\t', conservative, true, true, false), '\ta\t');
    assert.strictEqual(collapseWhitespace(' \t a \t ', conservative, true, true, false), ' a ');
  ...
  test('Ignores custom comments', async () => {
    let input;
  ...
  test('`preventAttributesEscaping`: Chooses a safe quote when the decoded value contains double quotes', async () => {
  ...
  test('`preventAttributesEscaping` with `quoteCharacter`: Handles quote conflicts safely', async () => {
  ...
  test('Removes space between attributes', async () => {
    let input, output;
  ...
  test('Collapses attribute whitespace', async () => {
    let input, output;
  ...
  test('Decodes entity characters', async () => {
    let input, output;
  ...
  test('`tbody` auto-closes open `tfoot`', async () => {
    // Per the HTML spec (“in table body” insertion mode), a `<tbody>` start tag
    // must close any currently open `tfoot` (or `thead`) in the same table
    const input = '<table><tfoot><tr><td>Footer</td></tr><tbody><tr><td>Body</td></tr></tbody></table>';
    const expected = '<table><tfoot><tr><td>Footer</td></tr></tfoot><tbody><tr><td>Body</td></tr></tbody></table>';
    assert.strictEqual(await minify(input, { includeAutoGeneratedTags: true }), expected);

    // Without auto-generated tags, the implicit `</tfoot>` is omitted
    assert.strictEqual(await minify(input, { includeAutoGeneratedTags: false }), input);
  });
  ...
  test('Synthetic `colgroup` auto-closes before non-`col` element', async () => {
    ...
    let input = '<table><col><tbody><tr><td>x</td></tr></tbody></table>';
    assert.strictEqual(await minify(input, { includeAutoGeneratedTags: true }),
      '<table><colgroup><col></colgroup><tbody><tr><td>x</td></tr></tbody></table>');
    assert.strictEqual(await minify(input, { includeAutoGeneratedTags: false }), input);

    // Multiple cols followed by table row
    input = '<table><col><col><tr><td>x</td><td>x</td></tr></table>';
    assert.strictEqual(await minify(input, { includeAutoGeneratedTags: true }),
      '<table><colgroup><col><col></colgroup><tr><td>x</td><td>x</td></tr></table>');
    assert.strictEqual(await minify(input, { includeAutoGeneratedTags: false }), input);
  });
  ...
  test('`partialMarkup`', async () => {
    ...
    // Test unclosed tags: Auto-generated closing tags are omitted by default
    input = '<div><p>Hello';
    assert.strictEqual(await minify(input), input,
      'without `partialMarkup`, auto-generated closing tags are omitted by default');
    assert.strictEqual(await minify(input, { includeAutoGeneratedTags: true }), '<div><p>Hello</p></div>',
      'with `includeAutoGeneratedTags`, unclosed tags should be auto-closed');
    assert.strictEqual(await minify(input, { partialMarkup: true }), input,
    ...
    input = '</br></p>';
    assert.strictEqual(await minify(input), '<br></p>',
    ...
    assert.strictEqual(await minify(input, { partialMarkup: true }), '<div><p>Text</div>',
  ...
  test('`sortAttributes` with `processScripts` does not corrupt HTML', async () => {
  ...
  test('`sortAttributes` with benchmark config does not corrupt HTML', async () => {
    const input = '<div id="test" class="foo">content</div>';
  ...
  test('`sortAttributes` with complex attributes does not cause parse errors', async () => {
  ...
  test('`plaintext` holds text to the end of the document', async () => {
    // Everything that follows the start tag is text, `</plaintext>` and all, and no end tag
    // ends the element

    const options = { collapseWhitespace: true, removeComments: true, removeAttributeQuotes: true, removeOptionalTags: true, decodeEntities: true };

    // What looks like markup is text, and stays as it is written—which is what the element is
    // there for, as what it holds is displayed as source
    let input = '<plaintext><b class="  y  ">a</b><!-- c --> a &amp;&amp; b';
    assert.strictEqual(await minify(input, options), input);
    // Minifying the output again leaves it alone
    assert.strictEqual(await minify(await minify(input, options), options), input);

    // An end tag of its own is text as well, and so is everything behind it
    input = '<plaintext>a</plaintext><b class="  y  ">b</b><!-- c -->';
    assert.strictEqual(await minify(input, options), input);
    assert.strictEqual(await minify('<plaintext>a</PLAINTEXT >b', options), '<plaintext>a</PLAINTEXT >b');

    // Character references stay as they are, as they do in any other raw text
    input = '<plaintext>a&amp;b&lt;c';
    assert.strictEqual(await minify(input, { decodeEntities: true }), input);

    // The element renders preformatted, so its whitespace stays; the trailing run at the end
    // of the document is trimmed as it is anywhere else
    assert.strictEqual(await minify('<plaintext>  a   b  ', { collapseWhitespace: true }), '<plaintext>  a   b');

    // The start tag ends an open paragraph, and the text stands outside it
    assert.strictEqual(await minify('<p>a<plaintext>b', { includeAutoGeneratedTags: true }), '<p>a</p><plaintext>b');
  ...
  test('Normalizes options that require conversion when coming from a preset', async () => {
