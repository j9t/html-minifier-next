...
describe('HTML', () => {
  ...
  // @@ Fix sortAttributes corruption with benchmark config
  // Issue: `sortAttributes` combined with certain options (especially `processScripts`)
  // causes HTML corruption where angle brackets become entities (`&lt;`), quotes get
  // escaped (`\"`), and extra spaces appear in the output
  ...
  test('sortAttributes with actual FAZ.html content', async () => {
    // Test with real FAZ.html file to ensure it minifies successfully
    // This is the ultimate test - if this passes, FAZ works
    const fs = await import('fs/promises');
    let fazHtml;
    try {
      fazHtml = await fs.readFile('benchmarks/sources/FAZ.html', 'utf8');
    } catch (err) {
      // Skip if FAZ.html not available
      console.log('Skipping FAZ.html test - file not found');
      return;
    }
    ...
    const benchmarkConfig = {
      ...
      removeTagWhitespace: true,
      sortAttributes: true,
      sortClassName: true, // Enable class sorting too
      trimCustomFragments: true,
      useShortDoctype: true
    };
    ...
    assert.ok(!result.includes('\\"') || result.match(/\\"/g).length < 10, 'Should not have excessive escaped quotes');
    assert.ok(!result.includes('&lt;html'), 'Should not encode html tag as entity');
