...
describe('HTML', () => {
  ...
  test('Remove optional tags in tables', async () => {
    ...
    // Without it, re-minifying stabilizes on a second pass (auto-generated `<tbody>` is not re-inserted)
    const stableOutput = '<table>' +
      '<thead><tr><th>foo<th>bar<th>baz' +
      '<tr><td>boo<td>moo<td>loo' +
      '<tfoot><tr><th>baz<th>qux<td>boo' +
      '</table>';
    assert.strictEqual(await minify(output, { collapseWhitespace: true, removeOptionalTags: true }), stableOutput);
    assert.strictEqual(await minify(stableOutput, { collapseWhitespace: true, removeOptionalTags: true }), stableOutput);
