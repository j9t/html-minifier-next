...
* Enabled [OXVG](https://github.com/noahbald/oxvg) as an opt-in engine for SVG minification, allowing to choose `oxvg` instead of `svgo` (the default)
  - **Important:** OXVG support is experimental, it’s installed separately (`npm i -D @oxvg/napi`, 0.0.8 or later), and it’s configured as a map of job names rather than in SVGO’s plugin format (an SVGO configuration handed to it is refused)
  - Like the SVGO path, OXVG runs with the jobs that treat an SVG as a standalone file turned off, unless the configuration names jobs of its own
  - **Important:** An internal error in OXVG ends the Node process rather than raising a catchable error, so `continueOnMinifyError` cannot absorb it and the rest of the run is lost; the README says how to tell such a run apart, find the SVG behind it, and get the build through
...
* Added API-only `canMinifyCSS`, `canMinifyJS`, and `canMinifySVG` hooks to exempt individual pieces of CSS, JavaScript, and SVG from the built-in minifiers
...
* Improved handling of `plaintext`, whose content is text to the end of the document
...
* Fixed the scan for the end of a comment, and of a downlevel-revealed conditional comment, rescanning the rest of the input at every opener that has no end (where `continueOnParseError` and a fragment such as `<!--` repeated cost minutes rather than milliseconds)
* Fixed `pre`, `table`, `main`, `nav`, `section`, `search`, `menu`, `listing`, `xmp`, `plaintext`, `center`, and `dir` not ending an open paragraph the way the spec has them, which left the paragraph open across the element and put the end tag `includeAutoGeneratedTags` writes behind it
* Fixed whitespace standing behind `textarea` being read as part of what the element keeps verbatim, which kept `collapseWhitespace` off it until a second run
* Fixed `decodeEntities` reading `textarea`, `title`, `iframe`, and `xmp` as elements whose content is text by name alone, where the namespace decides it—in SVG and MathML they hold markup, and escaping only their own end tag turned the text `&lt;b&gt;` into a live `<b>` element
* Fixed any `encoding` attribute on `annotation-xml` deciding whether the element holds HTML, where the first one alone does

## [8.1.1] - 2026-08-24

### Fixed

* Fixed `removeOptionalTags` having no effect wherever whitespace sat between tags, which made the option look like it only worked together with `collapseWhitespace`
* Fixed an optional end tag being omitted twice, once where it was written and once wherever the next end tag stood, which could drop markup that was never optional
* Fixed a `<tr>`, `<td>`, or `<th>` start tag not closing the row or cell it follows, which put the end tags `includeAutoGeneratedTags` writes for `<table><tr><td>a<tr><td>b</table>` at the end of the table rather than at the end of each row
...
* Fixed no-break spaces separating two consecutive `htmlmin:ignore` blocks being dropped, or collapsed into a single one, under `collapseWhitespace`
...
* Allowed `--input-dir` and `--output-dir` to point to the same directory
...
* Made sure preset options that require normalization (`minifyURLs`, `minifyCSS`, `minifyJS`, `minifySVG`, `caseSensitive`) are correctly converted; previously they were applied via `Object.assign` and bypassed normalization, causing a `TypeError` at runtime
...
* Fixed `</p>` end tag being incorrectly omitted inside custom element parents (per [spec](https://html.spec.whatwg.org/multipage/syntax.html#optional-tags), `</p>` cannot be omitted when the parent is an autonomous custom element)
...
[The following reads more complicated than it is. Consider going through [the brief upgrade guide](https://meiert.com/blog/html-minifier-next-5/).]
