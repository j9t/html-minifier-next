...
`customAttrAssign` and `customAttrSurround` patterns are merged into one attribute pattern, which carries no flags of its own. `i` and `s` are written into each pattern's source instead, so they survive the merge; `u` and `v` change how a source reads and cannot be, so a pattern carrying either is refused with an error rather than silently matching something else—a dropped `u` would leave `\p{L}` matching the literal text `p{L}`.
